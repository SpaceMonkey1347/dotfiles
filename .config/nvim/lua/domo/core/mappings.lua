...
local file_sourced = require(UTILS_PATH).mappings_loaded_flag
...
M.modify = {
    n = {
        ...
        -- TODO: ensure value github shorthand url and why it won't bind it gx
        ["<leader>gx"] = {
            function ()
                local ok, result = pcall(vim.cmd, "normal gx")
                if not ok then
                    local url = string.sub(result, 94, -4)
                    vim.ui.open("https://github.com/" .. url)
                end
            end,
        },
...
M.todo_comments = {
    ...
    n = {
        ["]tt"] = { function() require("todo-comments").jump_next() end, "Next todo comment (any)" },
        ["[tt"] = { function() require("todo-comments").jump_prev() end, "Prev todo comment (any)" },
